            ...
            VStack(alignment: .leading, spacing: 26) {
                ...
                ForEach(monitor.volumes) { volume in
                    VStack(alignment: .leading, spacing: 17) {
                        HStack(spacing: 15) {
                            Image(systemName: volume.isInternal ? "internaldrive.fill" : "externaldrive.fill").font(.system(size: 32)).foregroundStyle(Palette.green)
                            ...
                            Label(volume.status, systemImage: volume.isReadOnly ? "lock.fill" : volume.isLow ? "exclamationmark.circle.fill" : "circle.fill")
                                .font(.system(size: 10)).foregroundStyle(volume.isReadOnly || volume.isLow ? .orange : Palette.green)
                            Button { NSWorkspace.shared.open(volume.url) } label: { Image(systemName: "arrow.up.right.square") }.help("Open volume in Finder")
                        }
                        ProgressView(value: volume.usedFraction).tint(volume.isLow ? .orange : Palette.green)
